...
public class WordsSQLiteConnectionPrefsProvider implements PrefsProvider {
    ...
    public PrefsRoot getPrefsRoot() {
        ...
        Observable.fromIterable(mLocale)
                .map(locale -> new Pair<>(root.createChild().addValue("locale", locale), new WordsSQLiteConnection(mContext, mDatabaseFilename, locale)))
                .blockingSubscribe(pair -> pair.second.loadWords((word, frequency) -> {
                    pair.first.createChild()
                            .addValue("word", word)
    ...
    public void storePrefsRoot(PrefsRoot prefsRoot) throws Exception {
        Observable.fromIterable(prefsRoot.getChildren())
                ...
                .blockingSubscribe(pair -> pair.second.blockingSubscribe(prefItem -> {
                    pair.first.addWord(prefItem.getValue("word"), Integer.parseInt(prefItem.getValue("freq")));
                }));
    }
}
