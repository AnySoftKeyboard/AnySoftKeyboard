...
public class AnySoftKeyboardExtendingTest extends AnySoftKeyboardBaseTest {
    ...
    public void testAnySoftKeyboardClassHierarchy() throws Exception {
        ...
        final Set<Class<?>> allPossibleClasses =
                Collections.list(SoftKeyboard.class.getClassLoader().getResources(imePackage.replace('.', '/')))
                        .stream()
                        ...
                        .map(fullClassName -> {
                            try {
                                return Class.forName(fullClassName, true, SoftKeyboard.class.getClassLoader());
        ...
        while (!superclass.equals(AnySoftKeyboardBase.class)) {
            ...
            Assert.assertTrue("Class " + superclass + " is not in the allPossibleClasses set! Was it removed?", allPossibleClasses.remove(superclass));
            superclass = superclass.getSuperclass();
            Assert.assertNotNull(superclass);
        }

        final String errorMessage = "Still have classes in set: " + String.join(", ",
