...
public class OverlyDataCreatorForAndroid implements OverlyDataCreator {
    ...
    private static void fetchRemoteColors(OverlayData data, Context context, int themeId) {
        ...
        try {
            ...
            data.setPrimaryColor(getColorFromStyledAttribute(a, 0, resources));
            data.setPrimaryDarkColor(getColorFromStyledAttribute(a, 1, resources));
            data.setPrimaryTextColor(getColorFromStyledAttribute(a, 2, resources));
        } finally {
            a.recycle();
        }
    }

    private static int getColorFromStyledAttribute(TypedArray a, int index, Resources resources) {
