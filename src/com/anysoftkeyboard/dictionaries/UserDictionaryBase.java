...
public abstract class UserDictionaryBase extends EditableDictionary {
    ...
    public synchronized boolean addWord(String word, int frequency) {
        ...
        if (word.length() >= MAX_WORD_LENGTH)
            return false;
        Log.e(TAG, "Adding word '" + word + "' to dictionary (in "
                + getClass().getSimpleName() + ") with frequency " + frequency);
    ...
    protected void reloadDictionary() {
        try {
            ...
            Log.e(TAG, "Failed to load database while adding word (in "
    ...
    private boolean isValidWordRec(final NodeArray children,
                                   final CharSequence word, final int offset, final int length) {
            ...
            if (node.code == currentChar) {
                if (offset == length - 1) {
                    ...
                    if (node.children != null) {
                        if (isValidWordRec(node.children, word, offset + 1,
                                length)) {
                            return true;
                        }
                    }
     ...
     */
    private void getWordsRec(NodeArray roots, final WordComposer codes,
                             final char[] word, final int depth, boolean completion, float snr,
                             int inputIndex, WordCallback callback) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (completion) {
                ...
                if (children != null) {
                    getWordsRec(children, codes, word, depth + 1, completion,
                            snr, inputIndex, callback);
                }
                            ...
                            if (terminal) {
                                if (INCLUDE_TYPED_WORD_IF_VALID
                                    ...
                                    callback.addWord(
                                            ...
                                            depth + 1,
                                            (int) (freq * snr
                                                    * addedAttenuation * FULL_WORD_FREQ_MULTIPLIER));
                                }
                            }
                            if (children != null) {
                                getWordsRec(children, codes, word, depth + 1,
                                        true, snr * addedAttenuation,
                                        inputIndex + 1, callback);
                            }
                        } else if (children != null) {
                            getWordsRec(children, codes, word, depth + 1,
                                    false, snr * addedAttenuation,
                                    inputIndex + 1, callback);
                        }
                    }
                }
    ...
    protected synchronized final void addWordFromStorage(String word,
                                                         int frequency) {
        addWordRec(mRoots, word, 0, frequency);
    }
    ...
    private void addWordRec(NodeArray children, final String word,
        ...
        if (wordLength == depth + 1) {
            ...
            childNode.frequency += frequency; // If there are multiple similar
            // words
            return;
        }
     ...
     * Returns the word's frequency or -1 if not found
     */
    private int getWordFrequencyRec(final NodeArray children,
                                    final CharSequence word, final int offset, final int length) {
        final int count = children.length;
        char currentChar = word.charAt(offset);
        for (int j = 0; j < count; j++) {
            ...
            if (node.code == currentChar) {
                if (offset == length - 1) {
                    ...
                    if (node.children != null) {
                        int freq = getWordFrequencyRec(node.children, word,
                                offset + 1, length);
                        if (freq > -1)
                            return freq;
                    }
    ...
    // generated with:
    // cat UnicodeData.txt | perl -e 'while (<>) { @foo = split(/;/); $foo[5] =~
    // s/<.*> //; $base[hex($foo[0])] = hex($foo[5]);} for ($i = 0; $i < 0x500;
    // $i += 8) { for ($j = $i; $j < $i + 8; $j++) { printf("0x%04x, ",
    // $base[$j] ? $base[$j] : $j)}; print "\n"; }'
