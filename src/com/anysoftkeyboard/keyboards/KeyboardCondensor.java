...
public class KeyboardCondensor {
	...
	private final Map<Integer, KeySize> mKeySizesMap = new HashMap<Integer, KeySize>();
	...
	public void setCondensedKeys(boolean condensed) {
		...
		if (condensed)
		{
			...
			for(Key k : mKeyboard.getKeys())
			{
				...
				if (currentY != k.y)//on new line, we want to handle the left side of the keyboard
				{
					flipSideLeft = !flipSideLeft;
					
					condenseRightSide(CONDENSING_FACTOR, keyboardWidth, currentRightX, rightKeys, spaceKey);
				...
				if ((k.gap + k.x) < watershedLineX && k.codes[0] == KeyCodes.SPACE)
				{
					//space is a special case, I want to make it as wide as possible
					spaceKey = k;
					currentLeftX = condenseLeftSide(CONDENSING_FACTOR, currentLeftX, k, targetWidth);
				}
	...
	int condenseLeftSide(final float CONDENSING_FACTOR, int currentLeftX, Key k,
			int targetWidth) {
