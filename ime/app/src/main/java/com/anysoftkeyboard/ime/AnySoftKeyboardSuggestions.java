...
public abstract class AnySoftKeyboardSuggestions extends AnySoftKeyboardKeyboardSwitchedListener {
  ...
  public void pickSuggestionManually(
    ...
    // Set once the batch edit has been closed early, so the finally block does not close it
    // twice (the homophone branch has to close it before reading the cursor position).
    boolean batchEditClosed = false;
    ...
    try {
      ...
      // Homophone search is armed and the user picked from the radical candidate strip
      // instead of pressing space. Space has a dedicated branch in handleSeparator, so
      // mirror it here: commit the picked character and run the same lookup, otherwise
      // only the highlighted candidate would ever reach the homophone list.
      if (!isHomophonePick
          && mHomophoneSearchMode
          && isRadicalInputMode()
          && !TextUtils.isEmpty(actualChar)) {
        commitWordToInput(actualChar, actualChar);
        mWordRevertLength = 0;
        mJustAutoAddedWord = false;
        // Close the batch edit before looking up homophones: showHomophonesFor anchors on
        // getCursorPosition(), which can still report the pre-commit offset while the batch
        // is open. This mirrors what the space-key branch in handleSeparator does.
        if (ic != null) {
          ic.endBatchEdit();
          batchEditClosed = true;
        }
        showHomophonesFor(actualChar);
        return;
      }
