...
Dictionary::Dictionary(unsigned char *dict, int typedLetterMultiplier,
...
unsigned short Dictionary::toLowerCase(unsigned short c) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (completion) {
      ...
      while (currentChars[j] > 0) {
        const unsigned short currentChar =
            (const unsigned short)currentChars[j];
...
bool Dictionary::isValidWordRec(int pos, unsigned short *word, int offset,
                                int length) {
  int count = getCount(&pos);
