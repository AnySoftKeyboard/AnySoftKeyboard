...
Dictionary::Dictionary(void *dict, int typedLetterMultiplier,
...
int Dictionary::getSuggestions(int *codes, int codesSize,
...
int Dictionary::getAddress(int *pos) {
  int address = 0;
...
int Dictionary::getFreq(int *pos) { return mDict[(*pos)++] & 0xFF; }

int Dictionary::wideStrLen(unsigned short *str) {
  if (!str) return 0;
...
bool Dictionary::addWord(unsigned short *word, int length, int frequency,
                         bool isTyped) {
  word[length] = 0;
  ...
  if (insertAt < mMaxWords) {
    ...
    mFrequencies[insertAt] = frequency;
    memmove(
        (char *)mOutputChars + (insertAt + 1) * mMaxWordLength * sizeof(short),
...
bool Dictionary::sameAsTyped(unsigned short *word, int length) {
...
void Dictionary::getWordsRec(int pos, int depth, int maxDepth, bool completion,
                             int snr, int inputIndex, int diffs) {
  // Optimization: Prune out words that are too long compared to how much was
  // typed.
  if (depth > maxDepth) {
    return;
  }
  ...
  for (int i = 0; i < count; i++) {
    ...
    // If we are only doing completions, no need to look at the typed
    // characters.
    if (completion || currentChars == nullptr/*basically, the same since currentChars is null if completion is set to true*/) {
      ...
      if (childrenAddress != 0) {
        getWordsRec(childrenAddress, depth + 1, maxDepth, completion, snr,
                    inputIndex, diffs);
      }
    } else {
      int j = 0;
      while (currentChars[j] > 0) {
        ...
        const unsigned short lowerCurrentChar =
            CharUtils::toBaseLowerCase(currentChar);
        // currentChar can be upper or lower
        // c can be upper or lower
        // lowerC is lower or c (in the case where we do not know how to convert
        // to lower) lowerCurrentChar is lower or c  (in the case where we do
        // not know how to convert to lower) so, c must be checked against
        // currentChar (in cases where we do not know how to convert) and
        // lowerCurrent should be compared to lowerC (will verify the cases
        // where we do know how to convert)
        if (lowerCurrentChar == lowerC || currentChar == c) {
          ...
          if (mInputLength == inputIndex + 1) {
            ...
            if (childrenAddress != 0) {
              getWordsRec(childrenAddress, depth + 1, maxDepth, true,
                          snr * addedWeight, inputIndex + 1, diffs + (j > 0));
            }
          } else if (childrenAddress != 0) {
            getWordsRec(childrenAddress, depth + 1, maxDepth, false,
                        snr * addedWeight, inputIndex + 1, diffs + (j > 0));
          }
        }
        j++;
...
bool Dictionary::checkFirstCharacter(unsigned short *word) {
  // Checks whether this word starts with same character or neighboring
  // characters of what user typed.
...
bool Dictionary::isValidWord(unsigned short *word, int length) {
  ...
  if (!isValid) {
    ...
    const unsigned short lowerCaseFirstCharacter =
        CharUtils::toBaseLowerCase(word[0]);
    if (lowerCaseFirstCharacter == word[0]) return false;
...
void Dictionary::countWordsHelper(int pos, int depth, int &wordCount,
                                  int &wordsCharsCount, unsigned short *&words,
                                  int *&freqs) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (childrenAddress != 0) {
      countWordsHelper(childrenAddress, depth + 1, wordCount, wordsCharsCount,
                       words, freqs);
    }
  }
}

int Dictionary::isValidWordRec(int pos, unsigned short *word, int offset,
                               int length) {
  ...
  for (int j = 0; j < count; j++) {
    ...
    // There could be two instances of each alphabet - upper and lower case. So
    // continue looking ...
  }
