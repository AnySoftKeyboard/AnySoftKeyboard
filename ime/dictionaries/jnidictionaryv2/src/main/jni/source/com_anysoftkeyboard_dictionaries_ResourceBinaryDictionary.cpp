...
static jlong nativeime_ResourceBinaryDictionary_open(JNIEnv *env,
                                                     jobject object,
                                                     jobject dictDirectBuffer,
                                                     jint typedLetterMultiplier,
                                                     jint fullWordMultiplier) {
  ...
  Dictionary *dictionary =
      new Dictionary(dict, typedLetterMultiplier, fullWordMultiplier);
  return jlong(dictionary);
}

static int nativeime_ResourceBinaryDictionary_getSuggestions(
    JNIEnv *env, jobject object, jlong dict, jintArray inputArray,
    jint arraySize, jcharArray outputArray, jintArray frequencyArray,
    jint maxWordLength, jint maxWords, jint maxAlternatives, jint skipPos,
  ...
  if (frequencyArray == nullptr || inputArray == nullptr ||
      outputArray == nullptr)
    return 0;
  ...
  int *nextLetters = nextLettersArray != nullptr
                         ? env->GetIntArrayElements(nextLettersArray, nullptr)
                         : nullptr;

  if (!frequencies || !inputCodes || !outputChars ||
      (nextLettersArray != nullptr && !nextLetters)) {
    if (frequencies)
  ...
  int count = dictionary->getSuggestions(
      inputCodes, arraySize, outputChars, frequencies, maxWordLength, maxWords,
      maxAlternatives, skipPos, nextLetters, nextLettersSize);
...
static jboolean nativeime_ResourceBinaryDictionary_isValidWord(
    JNIEnv *env, jobject object, jlong dict, jcharArray wordArray,
    jint wordLength) {
  ...
  jboolean result =
      static_cast<jboolean>(dictionary->isValidWord(word, wordLength));
...
static jboolean nativeime_ResourceBinaryDictionary_getWords(
  ...
  jobjectArray javaLandChars =
      env->NewObjectArray(wordCount, charArrayClass, NULL);
  if (!javaLandChars) {
    env->ExceptionClear();
    return (jboolean) false;
  }
  ...
  env->CallVoidMethod(getWordsCallback, sGetWordsCallbackMethodId,
                      javaLandChars, frequencyArray);
  return (jboolean) true;
}

static void nativeime_ResourceBinaryDictionary_close(JNIEnv *env,
                                                     jobject object,
                                                     jlong dict) {
  Dictionary *dictionary = reinterpret_cast<Dictionary *>(dict);
  delete dictionary;
...
static JNINativeMethod gMethods[] = {
    {"openNative", "(Ljava/nio/ByteBuffer;II)J",
     (void *)nativeime_ResourceBinaryDictionary_open},
    {"closeNative", "(J)V", (void *)nativeime_ResourceBinaryDictionary_close},
    {"getSuggestionsNative", "(J[II[C[IIIII[II)I",
     (void *)nativeime_ResourceBinaryDictionary_getSuggestions},
    {"isValidWordNative", "(J[CI)Z",
     (void *)nativeime_ResourceBinaryDictionary_isValidWord},
    {"getWordsNative",
     "(JLcom/anysoftkeyboard/dictionaries/GetWordsCallback;)Z",
     (void *)nativeime_ResourceBinaryDictionary_getWords}};
...
static int registerNatives(JNIEnv *env) {
  const char *const kClassPathName =
      "com/anysoftkeyboard/dictionaries/jni/ResourceBinaryDictionary";
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
  ...
  jclass getWordsCallbackClass =
      env->FindClass("com/anysoftkeyboard/dictionaries/GetWordsCallback");
  // void (char[][] words, int[] frequencies);
  sGetWordsCallbackMethodId =
      env->GetMethodID(getWordsCallbackClass, "onGetWordsFinished", "([[C[I)V");
