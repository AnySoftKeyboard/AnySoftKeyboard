...
function isAddOnsFile(fileName:string): boolean {
    return supportedOddOns.some(postFix => fileName.endsWith(postFix[2]));
}

function getAddOnsFromXml(root): string[] {
...
export class UniqueAddOnIdChecker implements Checker {
    ...
    async check(root_dir: string, dir_skip_predicate: (dirname: string) => boolean): Promise<void> {
        ...
        return this.checkRec(root_dir, idSet, new XMLParser(parserOptions), dir_skip_predicate)
            .then(sum => console.log(`Finished without duplications (scanned ${sum} add-ons).`));
    }

    private async checkRec(root_dir: string, idSet: Set<string>, xmlParser: XMLParser, dir_skip_predicate: (dirname: string) => boolean): Promise<number> {
            ...
            const promises = files.map(file => {
                ...
                return fs.promises.stat(filePath).then(stat => {
                    if (stat.isDirectory()) {
                        ...
                        if (isAddOnsFile(file.toLowerCase())) {
                            return fs.promises.readFile(filePath, 'utf-8').then(xmlData => {
                                ...
                                try {
                                    for (const id of addOns) {
                                        if (filePath.indexOf("ime/app/src/main/res/xml") >= 0 && filePath.endsWith("/quick_text_keys.xml")) {
